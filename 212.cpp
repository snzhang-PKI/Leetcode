//simple DFS search, TLE
//prefix tree
//建好了树之后 遍历board而非树(前者更快) 这里是用了慢的算法 勉强在规定时间内通过了
...
public:
    vector<string> findWords(vector<vector<char>>& board, vector<string>& w) 
    {
        ...
        if (m)
        {
        	...
        	if (n)
        	{
        		unordered_map <char,list<pair<int,int>>> charLocation;
...
private:
	...
	bool findWord (vector<vector<char>> board, const string& word, int i, int j)
	{
		...
		{
			string tail = word.substr(1,l-1);
			if (InBoard(i-1,j,m,n) && board[i-1][j] == word[0])
			{
				auto newBoard = board;
				newBoard[i-1][j] = '*';
				if (findWord(newBoard, tail, i-1, j))
				{
					return true;
				}
			}
			...
			if (InBoard(i,j-1,m,n) && board[i][j-1] == word[0])
			{
				auto newBoard = board;
				newBoard[i][j-1] = '*';
				if (findWord(newBoard, tail, i, j-1))
				{
					return true;
				}
			}
...
int main(int argc, char const *argv[])
{
	...
	vector<string> vs = {"aaaa","aaaa","aaaa","aaaa","bcde","fghi","jklm","nopq","rstu","vwxy","zzzz"};
	for (auto str : vs)
	{
		vector <char> tmp;
		for (auto c : str)
